...
//        myWindow.writeOutLine(b);
    ...
    private void lab1(){
        ...
        int[] myIntergerArray = {4, 3, 2, 1};
//        int[] myIntergerArray = {4, 3, 2, 1, 7};

        // a
        int lastElementIndex = myIntergerArray.length - 1;
        myWindow.writeOutLine("The last element in the array is " + myIntergerArray[lastElementIndex]); // 1


        // b
        int[] a = myIntergerArray;
        ...
        myWindow.writeOutLine("The product of all the elements in the array is " + product); // 24


        // c
        boolean flag = Math.sqrt(myIntergerArray[0]) == myIntergerArray[2];
        myWindow.writeOutLine("The third element in the array is the square root of the first element: " + flag); // True
    }
    ...
    private void lab2(){
        ...
        // Compile Error: Variable has to be initialized
//        System.out.println(myStringArray); // Compile Error, reference variable is not initialized
        myStringArray = new String[3];
//        myWindow.writeOutLine(myStringArray); // No Error

        // Run-time Error: NullPointerException
//        System.out.println(myStringArray[0].length()); // Run-time Error, myStringArray[0] contains 'null'
        myStringArray[0] = "green";
        myWindow.writeOutLine("The length of the first element: " + myStringArray[0].length());

        // Run-time Error: Array Index out of Bound
//        System.out.println(myStringArray[myStringArray.length]); // Run-time Error
//        System.out.println(myStringArray[myStringArray.length-1]); // No Error
    }
    ...
    private void lab3(){
        ...
        myWindow.writeOutLine(myList); // [four, three, two, one]

        myList.set(2, "seven");
        myWindow.writeOutLine(myList); // [four, three, seven, one]

        myList.remove(0);
        myWindow.writeOutLine(myList); // [three, seven, one]

        myList.add(myList.size()-1, "five");
        myList.add(myList.size()-1, "five");
        myList.add(myList.size()-1, "five");
        myList.add(myList.size()-1, "five");
        myWindow.writeOutLine(myList); // [three, seven, five, five, five, five, one]

        myWindow.writeOutLine("The index of seven in the array is: " + myList.indexOf("seven")); // 1


        // b
        myWindow.writeOutLine("The array contains the string \"seven\": " + myList.contains("seven")); // True
    }



    private void lab4(){
        String word, firstLetterUpper, otherLettersLower;

        word = "CAtaStROPhe";
        firstLetterUpper = word.substring(0,1).toUpperCase();
        otherLettersLower = word.substring(1).toLowerCase();
        myWindow.writeOutLine("The converted version of " + word + " is: " + firstLetterUpper + otherLettersLower); // Catastrophe

        word = "gAbRiEl";
        firstLetterUpper = word.substring(0,1).toUpperCase();
        otherLettersLower = word.substring(1).toLowerCase();
        myWindow.writeOutLine("The converted version of " + word + " is: " + firstLetterUpper + otherLettersLower); // Gabriel

        word = "LoWeR";
        firstLetterUpper = word.substring(0,1).toUpperCase();
        otherLettersLower = word.substring(1).toLowerCase();
        myWindow.writeOutLine("The converted version of " + word + " is: " + firstLetterUpper + otherLettersLower); // Lower
    }
